...
export default function FinancialConsultant({ playAudio, stopAudio, isActive }) {
  ...
  // Consultas de datos reales para el mini-dashboard
  ...
  // Inicializar conversación con el agente central
  useEffect(() => {
  ...
  const quickQuestions = ["Estado de resultados", "Análisis de gastos", "¿Cuánto gané este mes?"];
    ...
    <div className="flex flex-col h-full gap-4">
      ...
      <div className="grid grid-cols-3 gap-2 shrink-0">
        <div className="bg-green-50 border border-green-100 p-3 rounded-xl">
          <p className="text-[10px] text-green-600 font-bold uppercase">Ventas</p>
          <p className="text-sm font-bold text-slate-800">${totalRevenue.toLocaleString()}</p>
        </div>
        <div className="bg-orange-50 border border-orange-100 p-3 rounded-xl">
          <p className="text-[10px] text-orange-600 font-bold uppercase">Gastos</p>
          <p className="text-sm font-bold text-slate-800">${totalExpenses.toLocaleString()}</p>
        </div>
        <div className="bg-blue-50 border border-blue-100 p-3 rounded-xl">
          <p className="text-[10px] text-blue-600 font-bold uppercase">Utilidad</p>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center opacity-40 text-center px-4">
            ...
            <p className="text-sm font-medium">Análisis de Finanzas y Rentabilidad</p>
